...
PE_IMPLEMENT_CLASS1(Event_CreateA6Character, PE::Events::Event_CREATE_SKELETON);

void Event_CreateA6Character::SetLuaFunctions(PE::Components::LuaEnvironment *pLuaEnv, lua_State *luaVM)
{
	static const struct luaL_Reg l_Event_CreateA6Character[] = {
		{ "Construct", l_Construct },
		{ NULL, NULL } // sentinel
	};

	// register the functions in current lua table which is the table for Event_CreateSoldierNPC
	luaL_register(luaVM, 0, l_Event_CreateA6Character);
}

int Event_CreateA6Character::l_Construct(lua_State* luaVM)
{
	PE::Handle h("EVENT", sizeof(Event_CreateA6Character));
	...
	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
	Event_CreateA6Character *pEvt = new(h)Event_CreateA6Character(pContext->m_gameThreadThreadOwnershipMask);
...
PE_IMPLEMENT_CLASS1(Event_A6C_Throttle, Event);
