...
void PositionBufferCPU::ReadPositionBuffer(const char *filename, const char *package)
{
	...
	// Read all values and calculating max and min of a mesh
	PrimitiveTypes::Float32 val;
	f.nextFloat32(val);
	m_values.add(val * factor);
	float maxXVal, minXVal;
	maxXVal = val * factor;// *factor;
	minXVal = val * factor;// *factor;

	f.nextFloat32(val);
	m_values.add(val * factor);
	float maxYVal, minYVal;
	maxYVal = val * factor;// *factor;
	minYVal = val * factor;// *factor;

	f.nextFloat32(val);
	m_values.add(val * factor);
	float maxZVal, minZVal;
	maxZVal = val * factor;// *factor;
	minZVal = val * factor;// *factor;

	for (int i = 3; i < n * 3; i += 3)
	{
		f.nextFloat32(val);
		if (maxXVal < (val * factor))
		{
			maxXVal = val * factor;//* factor;
		}
		else if (minXVal >(val * factor))
		{
			minXVal = val * factor;//*factor;
		}
		m_values.add(val * factor);
		f.nextFloat32(val);
		if (maxYVal < (val * factor))
		{
			maxYVal = val * factor;// *factor;
		}
		else if (minYVal >(val * factor))
		{
			minYVal = val * factor;// *factor;
		}
		m_values.add(val * factor);
		f.nextFloat32(val);
		if (maxZVal < (val * factor))
		{
			maxZVal = val * factor;//* factor;
		}
		else if (minZVal >(val * factor))
		{
			minZVal = val * factor;// *factor;
		}
		m_values.add(val * factor);
	}
	m_maxValues.add(maxXVal);
	m_maxValues.add(maxYVal);
	m_maxValues.add(maxZVal);

	m_minValues.add(minXVal);
	m_minValues.add(minYVal);
	m_minValues.add(minZVal);
}
